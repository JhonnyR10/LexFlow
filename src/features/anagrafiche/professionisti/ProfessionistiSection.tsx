...
function ContactsCell({ item }: { item: ProfessionistaListItem }): React.JSX.Element {
  const lines: string[] = []
  if (item.email) lines.push(item.email)
  if (item.pec)   lines.push(`PEC: ${item.pec}`)
  if (item.telefono) lines.push(item.telefono)
  if (lines.length === 0) return <span style={{ color: 'var(--color-text-muted)' }}>—</span>
  return (
    <>
      {lines.map((l, i) => (
        <span key={i} style={{ display: 'block', fontSize: '12px' }}>{l}</span>
      ))}
    </>
  )
}
  ...
  return (
    <div style={sectionStyle}>
      <div style={sectionHeaderStyle}>
        <span style={sectionTitleStyle}>Professionisti</span>
        <button style={btnPrimaryStyle} onClick={() => setCreateOpen(true)}>
          + Nuovo professionista
        </button>
      </div>

      {isLoading && <div style={loadingStyle}>Caricamento professionisti…</div>}
      ...
      {!isLoading && !error && professionisti && professionisti.length === 0 && (
        <div style={emptyStyle}>
          Nessun professionista. Creane uno con il pulsante in alto.
        </div>
      )}
          ...
          <tbody>
            {professionisti.map((item) => (
              <tr key={item.id} style={{ background: item.isActive ? 'inherit' : 'var(--color-bg)' }}>
                ...
                <td style={tdStyle}>
                  <span style={{ color: item.ruolo ? 'var(--color-text)' : 'var(--color-text-muted)' }}>
                    {item.ruolo ?? '—'}
                  </span>
                </td>
                <td style={tdStyle}>
                  <ContactsCell item={item} />
                </td>
                <td style={tdStyle}>
                  {item.isActive ? (
      ...
      {(createOpen || editItem) && (
        <ProfessionistaFormModal
          ...
          professionista={editItem ?? undefined}
          onClose={() => {
            setCreateOpen(false)
            setEditItem(null)
          }}

...
**Marker di sicurezza esterno (S14.1).** Lo stato del lock deve essere noto **prima** di aprire il DB (il DB potrà essere cifrato in S14.2 e comunque risiede dentro la cartella dati): vive quindi in `<userData>/security.json`, fuori dal DB, gestito da `main/config/securityMarker.ts`. Struttura: `{ lockEnabled: boolean, kdf: { salt, iterations, keylen, digest }, verifier }`. **Nessuna password in chiaro:** si conserva solo il `salt` e un `verifier` = `PBKDF2(password, salt, …)` (stdlib Node `crypto`); lo sblocco al boot ricalcola il verifier e lo confronta in `timingSafeEqual`. Impostare/cambiare/rimuovere la password aggiorna il marker (`lockEnabled`, `salt`, `verifier`). Con `lockEnabled=false` (default) il marker è assente o inerte e il boot è identico all'MVP. La derivazione della **chiave di cifratura** SQLCipher dalla stessa password è S14.2 (con separazione di dominio dal verifier).

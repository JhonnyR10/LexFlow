...
- **Backup automatico periodico** (alla chiusura e/o a intervallo configurabile) con rotazione delle ultime N copie: **S11.7** (scheduler nel main process). Il **backup automatico preventivo prima del reset** (**S11.4**) riusa `writeBackupZip` e scrive in automatico `pre-reset-<ts>.zip` sotto `backup.backupPath`; è **obbligatorio** (se fallisce, il reset non procede). Configurazione in `AppSettings` (vedi `02-data-model.md`).
